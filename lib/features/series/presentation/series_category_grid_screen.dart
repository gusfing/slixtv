...
class SeriesCategoryGridScreen extends ConsumerStatefulWidget {
  final Category category;
  final void Function(SeriesItem series)? onSeriesTap;

  const SeriesCategoryGridScreen({
    super.key,
  ...
  @override
  ConsumerState<SeriesCategoryGridScreen> createState() => _SeriesCategoryGridScreenState();
}

class _SeriesCategoryGridScreenState extends ConsumerState<SeriesCategoryGridScreen> {
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 200) {
      if (!_isLoadingMore && _hasMore && !_isLoading) {
        _fetchSeries(isLoadMore: true);
      }
    }
  }

  Future<void> _fetchSeries({bool isLoadMore = false}) async {
    if (isLoadMore) {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          if (_focusedSeries == null && _series.isNotEmpty) {
            _focusedSeries = _series.first;
          }
        });
      }
    } catch (e) {
                ...
                children: [
                  ...
                  Expanded(
                    child: _isLoading
                        ...
                        : _series.isEmpty
                            ? const Center(child: Text('No series found', style: TextStyle(color: Colors.white54)))
                            : GridView.builder(
                                ...
                                itemCount: _series.length + (_isLoadingMore ? 4 : 0),
                                itemBuilder: (context, index) {
                                  if (index >= _series.length) {
                                    return const Center(child: CircularProgressIndicator(color: AppColors.primary));
                                  }
                                  final seriesItem = _series[index];
                                  final isFocused = _focusedSeries?.id == seriesItem.id;

                                  return _GridSeriesCard(
                                    series: seriesItem,
                                    isFocused: isFocused,
                                    onFocus: () => setState(() => _focusedSeries = seriesItem),
                                    onTap: () => widget.onSeriesTap?.call(seriesItem),
                                  );
                                },
                              ),
                  ),
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  ClipRRect(
                                    ...
                                    child: _focusedSeries!.poster.isNotEmpty
                                        ? CachedNetworkImage(
                                            imageUrl: _focusedSeries!.poster,
                                  ...
                                  Wrap(
                                    ...
                                    children: [
                                      if (_focusedSeries!.year.isNotEmpty)
                                        Text(_focusedSeries!.year, style: const TextStyle(color: Colors.white70, fontSize: 14)),
                                      if (_focusedSeries!.rating.isNotEmpty && _focusedSeries!.rating != '0' && _focusedSeries!.rating != '0.0')
                                        Row(
                                          mainAxisSize: MainAxisSize.min,
                                          children: [
                                            const Icon(Icons.star_rounded, color: Colors.amber, size: 18),
                                            const SizedBox(width: 4),
                                            Text(_focusedSeries!.rating, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
                                          ],
                                        ),
                                    ],
                                  ),
                                  ...
                                  // Description
                                  if (_focusedSeries!.description.isNotEmpty) ...[
                                    Text(
                                      _focusedSeries!.description,
                                      style: const TextStyle(color: Colors.white54, fontSize: 14, height: 1.5),
                                    ),
                                    const SizedBox(height: 24),
                                  ],
                                  // Details
                                  _DetailRow('Genre', _focusedSeries!.genre.isNotEmpty ? _focusedSeries!.genre : widget.category.title),
                                  if (_focusedSeries!.director.isNotEmpty) _DetailRow('Director', _focusedSeries!.director),
                                  if (_focusedSeries!.actors.isNotEmpty) _DetailRow('Cast', _focusedSeries!.actors),
                                  const SizedBox(height: 24),
                                ],
                              ),
                            ),
...
class _GridSeriesCard extends StatefulWidget {
  final SeriesItem series;
  final bool isFocused;
  final VoidCallback onFocus;
  final VoidCallback onTap;

  const _GridSeriesCard({
  ...
  @override
  State<_GridSeriesCard> createState() => _GridSeriesCardState();
}

class _GridSeriesCardState extends State<_GridSeriesCard> {
  @override
  Widget build(BuildContext context) {
    return Focus(
